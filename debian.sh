...
if [ $(command -v curl) ]; then
	curl -Lvo .tmoe-linux.sh https://cdn.jsdelivr.net/gh/2moe/tmoe-linux@master/.mirror/manager
elif [ $(command -v aria2c) ]; then
	aria2c --console-log-level=warn --no-conf --allow-overwrite=true -o .tmoe-linux.sh https://cdn.jsdelivr.net/gh/2moe/tmoe-linux@master/.mirror/manager
elif [ $(command -v wget) ]; then
	wget -O .tmoe-linux.sh https://cdn.jsdelivr.net/gh/2moe/tmoe-linux@master/.mirror/manager
else
	#带三个#为docker容器专用
	###tuna_mirror
	DEPENDENCY_01='wget'
