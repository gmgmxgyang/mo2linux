...
check_tmoe_command() {
	...
	TMOE_TIPS_00="Welcome to tmoe linux manager v1.3359,type ${TMOE_TIPS_01} to start it."
}
#########################
tmoe_manager_env() {
...
choose_termux_color_scheme() {
	...
	if (whiptail --title "colors.properties" --yes-button "neon" --no-button "monokai" --yesno "Your colors.properties is empty,please choose color scheme of termux.\n请选择终端配色。" 9 50); then
		aria2c -d "${HOME}/.termux" --allow-overwrite=true -o "colors.properties" 'https://gitee.com/mo2/zsh/raw/master/share/colors/neon'
	else
		aria2c -d "${HOME}/.termux" --allow-overwrite=true -o "colors.properties" 'https://gitee.com/mo2/zsh/raw/master/share/colors/monokai.dark'
	fi
	...
	if [ ! -s "termux.properties" ]; then
		if (whiptail --title "termux.properties" --yes-button "yes" --no-button "no" --yesno "Your termux.properties is empty,do you want to creat it? It will modify the keyboard layout.\n是否需要创建termux.properties？这将会修改小键盘布局。" 10 50); then
			aria2c --allow-overwrite=true -o "termux.properties" 'https://gitee.com/mo2/zsh/raw/master/share/termux.properties'
		fi
	fi
