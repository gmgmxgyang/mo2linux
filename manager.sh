...
tmoe_manager_main_menu() {
	...
	11) bash -c "$(curl -fLsS 'https://gitee.com/mo2/zsh/raw/master/zsh.sh')" ;;
	12) download_vnc_apk ;;
	13) start_vscode ;;
	14) enable_root_mode ;;
...
tmoe_locale_settings() {
	...
	CONTAINER_LOCALE=$(
		whiptail --title "LOCALE SETTINGS" \
			...
			"10" "zh_CN.UTF-8 Chinese_China中国" \
...
linux_deploy_pulse_server() {
	...
	echo "脚本用法：ssh连接后，输入apt install -y curl;bash <(curl -L gitee.com/mo2/linux/raw/master/debian.sh)"
...
###################################################
same_arch_or_different_arch() {
	if (whiptail --title "您是想要同架构运行,还是跨架构呢？" --yes-button 'same同' --no-button 'across跨' --yesno "Do you want to run on the same architecture or across architectures?\n除向下兼容外,跨架构运行的效率可能偏低" 0 0); then
