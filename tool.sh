...
tmoe_locale_settings() {
	...
	if [ -e "${TMOE_LOCALE_FILE}" ]; then
		...
		#	sed -i "s@zh_CN@${TMOE_LANG_HALF}@" debian-i
		#else
...
check_tmoe_linux_desktop_link() {
	if [ ! -e "/usr/share/applications/tmoe-linux.desktop" ]; then
		...
		cd /usr/share/icons
		curl -Lv -o tmoe-linux.png 'https://gitee.com/mo2/linux/raw/master/.mirror/icon.png'
