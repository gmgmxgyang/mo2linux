...
tmoe_desktop_faq() {
	...
	cat <<-EOF
		    ------------------------
				Q:${YELLOW}不就是apt install或pacman -S,再不然就是dnf install，有什么难的？${RESET}
		...
		以deb系为例，推荐先测试的是debian(stable和sid)和Ubuntu lts(例如bionic和focal)。
		...
		尽管HyperV对于Debian和Ubuntu等大众发行版的兼容性极佳，但对于MX linux19.0以及LMDE4等小众发行版却会出现奇怪的BUG。
	...
	cat <<-EOF
				   ...
				   基于Ubuntu的Linux Mint主要分支版本，以及基于Debian的LMDE(Linux Mint Debian Edition)都是不错的选择呢！§(*￣▽￣*)§
					...
					alpine之所以如此高效和轻量化，是因为它有些东西是残缺，更贴切一点来说是非标准的，当你把缺失的东西补回来后，它是否还会像之前那样轻量化呢？
					这是个值得思考的问题。
					-----------------------------
					-----------------------------
	EOF
	...
	cat <<-EOF
		    -----------------------------
			Q:${YELLOW}为什么绝大多数的命令和文件夹名称都是跟debian有关,我觉得特别别扭。${RESET}
