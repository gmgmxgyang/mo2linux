...
check_tmoe_command() {
	...
	TMOE_TIPS_00="Welcome to tmoe linux tool v1.4293,type ${TMOE_TIPS_01} to start this tool."
	#勿改00变量
}
#########
gnu_linux_env() {
	...
	*)
		...
		if [ -e "${TMOE_GIT_DIR}/tool.sh" ]; then
			...
			if [ $(command -v curl) ]; then
				...
				aria2c --console-log-level=warn --no-conf --allow-overwrite=true -o /tmp/.tmoe-linux-tool.sh https://gitee.com/mo2/linux/raw/master/tool.sh
