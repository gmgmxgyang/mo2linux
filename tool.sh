...
tmoe_linux_tool_menu() {
	...
	tmoe_linux_tool_menu_zh() {
		TMOE_OPTION=$(
			whiptail --title "Tmoe-Tool running on ${OSRELEASE}(202010)" \
				--menu "Welcome to tmoe linux tool v1.3325,Type ${TMOE_TIPS_02} to start this tool.\nPlease use the enter and arrow keys to operate." 0 50 0 \
	...
	tmoe_linux_tool_menu_en() {
		TMOE_OPTION=$(
			whiptail --title "Tmoe-Tool running on ${OSRELEASE}(202010)" \
				--menu "Welcome to tmoe linux tool v1.3325,Type ${TMOE_TIPS_02} to start it.\nPlease use the enter and arrow keys to operate." 0 50 0 \
...
tmoe_linux_tool_upgrade() {
	...
	if [ ! -h "${DEBIAN_I_FILE}" ]; then
		#	case "${LINUX_DISTRO}" in
		#	alpine) wget -O ${DEBIAN_I_FILE} 'https://gitee.com/mo2/linux/raw/master/tool.sh' ;;
		#	*) curl -Lv -o ${DEBIAN_I_FILE} 'https://gitee.com/mo2/linux/raw/master/tool.sh' ;;
		#	esac
