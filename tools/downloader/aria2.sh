...
tmoe_aria2_onekey() {
    ...
    aria2c --no-conf --allow-overwrite=true -o aria2.conf 'https://gitee.com/mo2/linux/raw/master/.config/aria2.conf'
    case ${TMOE_PROOT} in
...
exit 0
    ...
    cd ${TMOE_ARIA2_PATH}
    if [ ! -e "dht.dat" ]; then
        aria2c --no-conf --allow-overwrite=true -o dht.dat https://gitee.com/mo2/linux/raw/master/.config/dht.dat
        chmod 666 dht.dat
    fi
    if [ ! -e "dht6.dat" ]; then
        aria2c --no-conf --allow-overwrite=true -o dht6.dat https://gitee.com/mo2/linux/raw/master/.config/dht6.dat
        chmod 666 dht6.dat
    fi
...
creat_aria_ng_desktop_link() {
    ...
    if [ ! -e "/usr/share/icons/ariang.png" ]; then
        if [ -e "${ARIA_NG_ICON}" ]; then
            ...
            aria2c --no-conf --allow-overwrite=true -d /usr/share/icons -o ariang.png https://gitee.com/mo2/linux/raw/master/.mirror/ariang.png
        fi
    fi
